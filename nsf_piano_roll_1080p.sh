...
track_index="${2:-1}"
requested_duration_seconds="${3:-60}"
fade_duration="${4:-4}"
video_end_padding="${5:-1}"
...
cargo run --release -- cartridge "$nsf_file" track $track_index video pianoroll "$raw_video_file" audio "$raw_audio_file" frames $duration_frames

echo "=== Converting $raw_video_file and $raw_audio_file to $final_output ... ==="
# piano roll settings
ffmpeg \
  -y -f rawvideo -pix_fmt rgb24 -s 1920x1080 -r 60.0988 -i "$raw_video_file" \
