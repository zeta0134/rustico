...
var is_touch_detected = false;
var touch_button_elements = [];
var dpad_elements = [];
var active_touches = {};
...
function process_active_touch_regions() {
  ...
  for (let touch_identifier in active_touches) {
    let active_touch = active_touches[touch_identifier];
...
function updateTouchKeys() {
  ...
  let p1_up_left = p1_keys & (KEY_UP | KEY_LEFT);
  let p1_down_right = p1_up_left << 1;
  let p1_down_right_mask = p1_down_right ^ 0xFF;

  touch_keys[0] = p1_keys & p1_down_right_mask;

  onchange_callback(touch_keys[0], touch_keys[1]);
}

touch_input.register_button = function(querystring) {
...
touch_input.register_d_pad = function(querystring) {
...
touch_input.initialize_touch = function(root_element_querystring) {
  var touch_root_element = document.querySelector(root_element_querystring);
...
touch_input.p1_keys = function() {
	return touch_keys[0];
}

touch_input.p2_keys = function() {
	return touch_keys[1];
}
